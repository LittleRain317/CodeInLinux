...
static int (*_(sort_origin_cmp)) (const __list_t*, const __list_t*) = NULL;
...
void
_(sort) (__list* this, int (*cmp)(const __list_t*, const __list_t*))
{
	size_t i, size;
	__list_t** array;
	_(iterator) it;
	...
	array = (__list_t * *)malloc(sizeof(__list_t*) * size);

	it = this->begin(this);
	for (i = 0; it != this->end(this); ++i) {
		array[i] = it;
		it = this->next(this, it);
	}

	_(sort_origin_cmp) = cmp;
	qsort(array, size, sizeof(__list_t*), _(sort_cmp));

	node = __nodeptr(array[0]);
	this->head->next = node;
	node->prev = this->head;
	node->next = __nodeptr(array[1]);

	node = __nodeptr(array[size - 1]);
	this->tail->prev = node;
	node->next = this->tail;
	node->prev = __nodeptr(array[size - 2]);

	for (i = 1; i < size - 1; ++i) {
		node = __nodeptr(array[i]);
		node->prev = __nodeptr(array[i - 1]);
		node->next = __nodeptr(array[i + 1]);
	}
