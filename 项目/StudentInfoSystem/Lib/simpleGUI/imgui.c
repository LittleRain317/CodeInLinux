...
int textbox(int id, double x, double y, double w, double h, char textbuf[], int buflen)
{
	...
	double textPosY = y + h - 0.2 - GetFontAscent() / 2;//y+h/2-GetFontAscent()/2;
	//mystring buf[1024];
	//mystring* re = NULL;
	//int i = 0;
	//for (i = 0; i != 1024; ++i)
	//	mystring_init(&buf[i]);
	//mystring_init(&temp);
	//temp.assign(&temp, textbuf);
	//re = temp.split(&temp, 0, temp.find(&temp, 0, '\n'));
	//buf[0].assign(&buf[0], re->c_str(re));
	if ( inBox(gs_UIState.mousex, gs_UIState.mousey, x, x + w, y, y + h) ) 
	{
